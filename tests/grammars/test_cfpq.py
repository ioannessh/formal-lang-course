...
from project.grammars.cfpq import (
    cfpq_matrix,
    hellings,
    matrix,
    cfpq_hellings,
...
def test_context_free_path_querying_algorithm(cfg_text, graph, exp_set):
    ...
    assert cfpq_tensor(CFG.from_text(cfg_text), graph) == exp_set
    new_exp_set = {(v, t, u) for v, t, u in exp_set if v == 0 and u == 2}
    assert cfpq_tensor(CFG.from_text(cfg_text), graph, {0}, {2}) == new_exp_set
